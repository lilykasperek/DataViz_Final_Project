...
frequent <- pol_donations %>% group_by(Owner) %>%
  ...
  mutate(order = fct_reorder(.f = Owner, .x = donations))
 
plot1 <- ggplot(data = frequent, aes(x = amount, y = order, label = Amount, colour = League)) +
  geom_point() +
...
plot2 <- ggplot(data = high_donations, aes(x = amount, y = owner_ordered, label = Recipient, colour = party)) +
  geom_point() +
